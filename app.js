    ...
    if (isAdmin && content.status === 'pending') {
        adminActions = `
                    <div style="margin-top:12px;display:flex;gap:8px;">
                        <button class="btn-sm approve" onclick="reviewContent(${content.id}, 'approved');navigateTo('home')" style="padding:6px 18px;border-radius:10px;background:#dcfce7;color:#166534;border:1px solid #86efac;font-weight:600;"><svg viewBox="0 0 24 24"><polyline points="20 6 9 17 4 12"/></svg>通过</button>
                        <button class="btn-sm reject" onclick="reviewContent(${content.id}, 'rejected');navigateTo('home')" style="padding:6px 18px;border-radius:10px;background:#fee2e2;color:#991b1b;border:1px solid #fca5a5;font-weight:600;"><svg viewBox="0 0 24 24"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>拒绝</button>
                        <button class="btn-sm" onclick="toggleRecommend(${content.id}, ${content.is_recommended || false})" style="padding:6px 18px;border-radius:10px;background:#fef3c7;color:#d97706;border:1px solid #fcd34d;">${content.is_recommended ? '⭐ 取消推荐' : '⭐ 推荐'}</button>
                    </div>
                `;
    } else if (isAdmin) {
        adminActions = `
...
function openReportDetail(reportId) {
    ...
    const detailHtml = `
                <div style="margin-bottom:16px;">
                    ...
                    <div style="font-size:13px;color:#94a3b8;margin-top:4px;">被举报内容：<strong>${report.content_title || '已删除'}</strong></div>
                    <div style="font-size:13px;color:#94a3b8;margin-top:4px;">举报理由：</div>
                ...
                <div style="display:flex;gap:8px;flex-wrap:wrap;">
                    <button class="btn-sm approve" onclick="resolveReport(${report.id}, 'resolved', 'content_deleted')" style="padding:8px 16px;border-radius:10px;background:#dcfce7;color:#166534;border:1px solid #86efac;font-weight:600;">🗑️ 删除内容</button>
                    <button class="btn-sm" onclick="resolveReport(${report.id}, 'resolved', 'user_banned')" style="padding:8px 16px;border-radius:10px;background:#fee2e2;color:#991b1b;border:1px solid #fca5a5;font-weight:600;">🔒 封禁用户</button>
                    <button class="btn-sm" onclick="resolveReport(${report.id}, 'resolved', 'upload_blocked')" style="padding:8px 16px;border-radius:10px;background:#fef3c7;color:#d97706;border:1px solid #fcd34d;font-weight:600;">🚫 限制上传</button>
                    <button class="btn-sm" onclick="resolveReport(${report.id}, 'dismissed', null)" style="padding:8px 16px;border-radius:10px;background:#f1f5f9;color:#475569;border:1px solid #e2e8f0;">❌ 驳回</button>
                </div>
            `;
...
//  初 始 化
// ============================================================
console.log('👁️ 必看网 v14.2.0 (全SVG图标版)');
