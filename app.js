...
let selectedFriendId = null; // 用于私聊
let friendsList = [];
let chatMessages = [];
let collections = [];
let watchHistory = [];
...
function navigateTo(page, id) {
    ...
    if (page === 'chat' && id) {
        ...
        url.searchParams.set('uid', id);
        url.searchParams.delete('id');
        window.history.pushState({ page: 'chat', uid: id }, '', url);
        loadChat(id);
        return;
    }
...
// ============================================================
//  加 载 全 局 公 告（只显示一次，点击X后不再显示）
// ============================================================
let globalAnnouncementDismissed = false;

async function loadGlobalAnnouncement() {
    if (globalAnnouncementDismissed) {
...
function closeGlobalBanner() {
    globalAnnouncementDismissed = true;
    globalBanner.style.display = 'none';
    // 可以选择保存到 localStorage 永久不再显示
    localStorage.setItem('globalBannerDismissed', 'true');
}

// 检查是否之前已关闭
(function() {
    if (localStorage.getItem('globalBannerDismissed') === 'true') {
        globalAnnouncementDismissed = true;
    }
})();
        ...
        for (let report of data) {
            ...
            try {
                const { data: contentData } = await supabaseClient
                    .from('contents')
                    .select('title, description, file_url, url, category')
                    .eq('id', report.content_id)
                    .single();
                report.content_title = contentData?.title || '已删除内容';
                report.content_description = contentData?.description || '';
                report.content_file_url = contentData?.file_url || '';
                report.content_url = contentData?.url || '';
                report.content_category = contentData?.category || '';
            } catch (e) {
                report.content_title = '已删除内容';
                report.content_description = '';
                report.content_file_url = '';
                report.content_url = '';
...
async function loadFriends() {
    ...
    const { data, error } = await supabaseClient
        .from('friends')
        .select('friend_id, status, created_at, profiles!friend_id(id, nickname, avatar_url)')
        .eq('user_id', currentUser.id)
        .eq('status', 'accepted');
    if (data) {
...
async function checkUserBanned(userId) {
    ...
    if (data?.is_banned) {
        ...
        if (!expires || expires > new Date()) {
            return { banned: true, reason: data.ban_reason, expires: expires };
        }
    }
    return { banned: false };
}
...
async function ensureNotBanned() {
    const status = await checkUserBanned(currentUser.id);
    if (status.banned) {
        alert('你的账号已被封禁，无法执行此操作');
        return false;
    }
    return true;
}
    ...
    if (status.banned) {
        contentRender.innerHTML = `
            <div class="empty-state">
                ...
                <p style="color:#64748b;margin-top:8px;">${status.reason || '违规操作'}</p>
                ${status.expires ? `<p style="color:#64748b;font-size:13px;margin-top:4px;">封禁至：${status.expires.toLocaleString()}</p>` : '<p style="color:#64748b;font-size:13px;margin-top:4px;">永久封禁</p>'}
...
function renderContentList(data) {
    ...
    data.forEach(item => {
        ...
        let extraHtml = '';
        if (item.file_url) {
            const isImage = isImageFile(item.file_name);
            extraHtml = `
                <div class="file-info">
        ...
        if (currentPage === 'upload') {
            if (item.status === 'pending') {
                statusBadge = `<span class="status-badge pending">
                    <svg viewBox="0 0 24 24" width="14" height="14" stroke="#d97706" fill="none" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
                    待审核
                </span>`;
            } else if (item.status === 'approved') {
                statusBadge = `<span class="status-badge approved">
                    <svg viewBox="0 0 24 24" width="14" height="14" stroke="#166534" fill="none" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
                    已通过
                </span>`;
            } else if (item.status === 'rejected') {
                statusBadge = `<span class="status-badge rejected">
                    <svg viewBox="0 0 24 24" width="14" height="14" stroke="#991b1b" fill="none" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
                    已拒绝
                </span>`;
            }
        }
        ...
        if (showActions && currentPage === 'review' && isAdmin) {
            actionButtons = `
                <div class="actions">
                    <button class="btn-sm approve" onclick="reviewContent(${item.id}, 'approved')">
                    ...
                    <button class="btn-sm reject" onclick="reviewContent(${item.id}, 'rejected')">
                        <svg viewBox="0 0 24 24" width="14" height="14" stroke="currentColor" fill="none" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
                        拒绝
                    </button>
                    <button class="btn-sm" onclick="toggleRecommend(${item.id}, ${item.is_recommended || false})">
                        ${item.is_recommended ? 
                            '<svg viewBox="0 0 24 24" width="14" height="14" stroke="currentColor" fill="none" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg> 取消推荐' : 
                            '<svg viewBox="0 0 24 24" width="14" height="14" stroke="currentColor" fill="none" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg> 推荐'
                        }
                    </button>
                    <button class="btn-sm" onclick="viewContentDetail(${item.id})">
            ...
            actionButtons = `
                <div class="actions">
                    <button class="btn-sm" onclick="deleteContent(${item.id}, '${item.file_url || ''}')">
                        <svg viewBox="0 0 24 24" width="14" height="14" stroke="currentColor" fill="none" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 01-2 2H7a2 2 0 01-2-2V6m3 0V4a2 2 0 012-2h4a2 2 0 012 2v2"/></svg>
                        删除
                    </button>
                </div>
            `;
        ...
        if (currentPage === 'home' && isAdmin) {
            actionButtons += `
                <div class="actions" style="margin-top:4px;">
                    <button class="btn-sm" onclick="toggleRecommend(${item.id}, ${item.is_recommended || false})" style="font-size:11px;">
        ...
        // 收藏按钮
        const isCollected = collections.some(c => c.content_id === item.id);
        const collectBtn = `
            <button class="report-btn" onclick="event.stopPropagation();toggleCollect(${item.id})" title="${isCollected ? '取消收藏' : '收藏'}" style="right:44px;color:${isCollected ? '#ef4444' : '#94a3b8'};">
                <svg viewBox="0 0 24 24" width="16" height="16" stroke="currentColor" fill="${isCollected ? '#ef4444' : 'none'}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20.84 4.61a5.5 5.5 0 00-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 00-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 000-7.78z"/></svg>
            </button>
        `;
...
//  收 藏 功 能
// ============================================================
async function toggleCollect(contentId) {
    if (!currentUser) return;
    const isCollected = collections.some(c => c.content_id === contentId);
    if (isCollected) {
        ...
        collections = collections.filter(c => c.content_id !== contentId);
    } else {
        await supabaseClient
            .from('collections')
            .insert({ user_id: currentUser.id, content_id: contentId });
        collections.push({ content_id: contentId, created_at: new Date().toISOString() });
    }
    renderContents(); // 刷新显示
}
...
//  观 看 历 史 记 录
// ============================================================
async function addWatchHistory(contentId) {
    if (!currentUser) return;
    // 避免重复记录
    const exists = watchHistory.some(h => h.content_id === contentId);
    if (exists) {
        // 更新观看时间
        await supabaseClient
            .from('watch_history')
...
function renderDetail(content, isLiked, recs) {
    ...
    let extraHtml = '';
    if (content.file_url) {
        const isImage = isImageFile(content.file_name);
        extraHtml = `
...
async function loadUserProfileView(userId) {
    ...
    const addFriendBtn = userId !== currentUser.id ? `
        <button class="btn-sm" onclick="addFriend('${userId}')" style="margin-top:8px;padding:4px 16px;background:#6366f1;color:#fff;border:none;border-radius:8px;">
            ${isFriend ? '✅ 已是好友' : '➕ 添加好友'}
        </button>
    ` : '';
...
async function addFriend(friendId) {
    ...
    if (error) {
        ...
        await supabaseClient.from('notifications').insert({
            user_id: friendId,
            type: 'friend_request',
            content: `👤 ${currentUser.email} 请求添加你为好友`,
            link: `/messages`
        });
        loadFriends();
    }
...
async function loadChat(friendId) {
    ...
    // 渲染聊天界面
    const html = `
        <div class="detail-container" style="max-width:700px;">
            <div class="back-btn" onclick="navigateTo('messages')">
                <svg viewBox="0 0 24 24"><polyline points="15 18 9 12 15 6"/></svg>
                返回消息
            </div>
            <div class="detail-card" style="padding:16px;">
                <div style="display:flex;align-items:center;gap:12px;margin-bottom:16px;padding-bottom:12px;border-bottom:1px solid #eef2f6;">
                    <div class="avatar-sm" style="width:40px;height:40px;font-size:18px;background:#6366f1;color:#fff;border-radius:50%;display:flex;align-items:center;justify-content:center;overflow:hidden;">
                        ${friendProfile?.avatar_url ? `<img src="${friendProfile.avatar_url}" style="width:100%;height:100%;object-fit:cover;">` : (friendProfile?.nickname?.charAt(0) || 'U')}
                    </div>
                    <div style="font-weight:600;font-size:16px;">${friendProfile?.nickname || '好友'}</div>
                    <button onclick="shareResourceInChat('${friendId}')" style="margin-left:auto;padding:4px 12px;background:#6366f1;color:#fff;border:none;border-radius:8px;font-size:12px;">📤 分享资源</button>
                </div>
                <div id="chatMessages" style="max-height:400px;overflow-y:auto;margin-bottom:12px;padding:0 4px;">
                    ${chatMessages.map(msg => {
                        const isMine = msg.sender_id === currentUser.id;
                        return `
                            <div style="display:flex;justify-content:${isMine ? 'flex-end' : 'flex-start'};margin-bottom:8px;">
                                <div style="max-width:70%;background:${isMine ? '#6366f1' : '#f1f5f9'};color:${isMine ? '#fff' : '#1e293b'};padding:8px 14px;border-radius:12px;word-break:break-word;font-size:14px;">
                                    ${msg.content}
                                    ${msg.shared_content_id ? `<div style="font-size:12px;opacity:0.8;margin-top:4px;">📎 分享了资源</div>` : ''}
                                    <div style="font-size:10px;opacity:0.6;margin-top:4px;">${timeAgo(msg.created_at)}</div>
                                </div>
                            </div>
                        `;
                    }).join('')}
                    ${chatMessages.length === 0 ? '<div style="text-align:center;color:#94a3b8;padding:20px;">暂无消息，开始聊天吧</div>' : ''}
                </div>
                <div style="display:flex;gap:8px;">
                    <input type="text" id="chatInput" placeholder="输入消息..." style="flex:1;padding:8px 14px;border:2px solid #e2e8f0;border-radius:12px;font-size:14px;" onkeydown="if(event.key==='Enter')sendMessage('${friendId}')">
                    <button onclick="sendMessage('${friendId}')" style="padding:8px 18px;background:#6366f1;color:#fff;border:none;border-radius:12px;font-weight:600;">发送</button>
                </div>
            </div>
        </div>
    `;
...
async function sendMessage(receiverId) {
    ...
    await supabaseClient.from('notifications').insert({
        ...
        content: `💬 ${currentUser.email} 给你发了一条私聊消息`,
        link: `/chat?uid=${currentUser.id}`
    });
...
async function shareResourceInChat(friendId) {
    // 弹窗让用户选择要分享的资源
    ...
    // 简易选择：弹窗列出资源
    const choices = contents.map((c, i) => `${i+1}. ${c.title}`).join('\n');
    const choice = prompt(`选择要分享的资源（输入编号）：\n${choices}`);
    if (!choice) return;
    const idx = parseInt(choice) - 1;
    if (isNaN(idx) || idx < 0 || idx >= contents.length) {
        alert('无效选择');
        return;
    }
    const selected = contents[idx];
    ...
    await supabaseClient
        ...
        .insert({
            ...
            content: `📤 分享了资源：${selected.title}`,
            shared_content_id: selected.id
        });
...
function renderMessages() {
    ...
    (async function() {
        const { data: friendRequests } = await supabaseClient
            .from('friends')
            .select('user_id, created_at, profiles!user_id(id, nickname, avatar_url)')
            .eq('friend_id', currentUser.id)
            .eq('status', 'pending');

        let html = `
            <div style="max-width:700px;margin:0 auto;">
                <h2 style="font-size:22px;color:#0f172a;margin-bottom:16px;">💬 消息</h2>
                <!-- 系统消息 -->
                <div style="margin-bottom:16px;">
                    <h3 style="font-size:16px;color:#0f172a;margin-bottom:8px;">📢 系统消息</h3>
                    ${allNotifications.filter(n => n.type === 'system').length === 0 ? '<p style="color:#94a3b8;font-size:14px;">暂无系统消息</p>' : ''}
                    ${allNotifications.filter(n => n.type === 'system').map(n => `
                ...
                <!-- 活动消息 -->
                <div style="margin-bottom:16px;">
                    <h3 style="font-size:16px;color:#0f172a;margin-bottom:8px;">🎉 活动消息</h3>
                    ${allNotifications.filter(n => n.type === 'activity').length === 0 ? '<p style="color:#94a3b8;font-size:14px;">暂无活动消息</p>' : ''}
                    ${allNotifications.filter(n => n.type === 'activity').map(n => `
                        <div class="notification-item ${n.is_read ? '' : 'unread'}" onclick="markNotificationRead('${n.id}')">
                            <div class="noti-content">
                                <div class="noti-text">${n.content}</div>
                                <div class="noti-time">${timeAgo(n.created_at)}</div>
                ...
                <!-- Bug反馈 -->
                <div style="margin-bottom:16px;">
                    <h3 style="font-size:16px;color:#0f172a;margin-bottom:8px;">🐛 Bug反馈</h3>
                    <div style="display:flex;gap:8px;margin-bottom:8px;">
                        <input type="text" id="bugFeedbackInput" placeholder="描述你遇到的问题..." style="flex:1;padding:8px 14px;border:2px solid #e2e8f0;border-radius:12px;font-size:14px;">
                        <button onclick="submitBugFeedback()" style="padding:8px 18px;background:#6366f1;color:#fff;border:none;border-radius:12px;font-weight:600;">提交</button>
                    </div>
                    <p style="font-size:12px;color:#94a3b8;">每天只能提交1条反馈</p>
                    <div id="bugFeedbackList">
                        ${allNotifications.filter(n => n.type === 'bug_feedback').map(n => `
                            <div class="notification-item ${n.is_read ? '' : 'unread'}" onclick="markNotificationRead('${n.id}')">
                                <div class="noti-content">
                                    <div class="noti-text">${n.content}</div>
                                    <div class="noti-time">${timeAgo(n.created_at)}</div>
                ...
                <!-- 好友 -->
                <div style="margin-bottom:16px;">
                    <h3 style="font-size:16px;color:#0f172a;margin-bottom:8px;">👥 好友</h3>
                    ${friendRequests && friendRequests.length > 0 ? `
                        <div style="margin-bottom:12px;padding:10px;background:#f8fafc;border-radius:10px;">
                            <div style="font-weight:600;font-size:14px;color:#0f172a;">好友请求</div>
                            ${friendRequests.map(req => `
                                <div style="display:flex;justify-content:space-between;align-items:center;padding:6px 0;border-bottom:1px solid #eef2f6;">
                                    <span>${req.profiles?.nickname || '用户'}</span>
                                    <button class="btn-sm approve" onclick="acceptFriend('${req.user_id}')" style="padding:2px 12px;background:#dcfce7;color:#166534;border:1px solid #86efac;border-radius:8px;">接受</button>
                                </div>
                            `).join('')}
                        </div>
                    ` : ''}
                    ${friendsList.length === 0 ? '<p style="color:#94a3b8;font-size:14px;">暂无好友</p>' : ''}
                    ${friendsList.map(f => `
                        <div class="notification-item" onclick="navigateTo('chat', '${f.id}')" style="cursor:pointer;">
                            <div style="display:flex;align-items:center;gap:10px;flex:1;">
                                <div class="avatar-sm" style="width:32px;height:32px;border-radius:50%;background:#6366f1;color:#fff;display:flex;align-items:center;justify-content:center;overflow:hidden;font-size:14px;font-weight:600;">
                                    ${f.avatar_url ? `<img src="${f.avatar_url}" style="width:100%;height:100%;object-fit:cover;">` : (f.nickname?.charAt(0) || 'U')}
                                </div>
                                <span style="font-weight:500;">${f.nickname}</span>
                            </div>
...
async function submitBugFeedback() {
    ...
    const { data: existing } = await supabaseClient
        ...
        .select('created_at')
        .eq('user_id', currentUser.id)
        .eq('type', 'bug_feedback')
        .gte('created_at', new Date(today).toISOString())
        .limit(1);
    if (existing && existing.length > 0) {
        alert('今天已经提交过反馈，请明天再试');
        return;
    }
    ...
    if (admins) {
        for (const admin of admins) {
            await supabaseClient.from('notifications').insert({
                user_id: admin.id,
                type: 'bug_feedback',
                content: `🐛 Bug反馈来自 ${currentUser.email}：${content}`,
                link: '/messages'
            });
        }
        // 给自己也留一条记录
...
function openReportDetail(reportId) {
    ...
    if (report.content_title) {
        contentPreview += `<div style="font-weight:600;margin:8px 0;">📄 ${report.content_title}</div>`;
    }
    if (report.content_description) {
        contentPreview += `<div style="color:#475569;font-size:14px;margin-bottom:6px;">${report.content_description}</div>`;
    }
    if (report.content_file_url) {
        const isImage = isImageFile(report.content_file_url);
        contentPreview += `<div style="background:#f8fafc;padding:8px;border-radius:8px;margin-bottom:6px;">
            <a href="${report.content_file_url}" target="_blank">📎 查看文件</a>
            ${isImage ? `<div style="margin-top:6px;"><img src="${report.content_file_url}" style="max-width:100%;max-height:200px;border-radius:4px;"></div>` : ''}
        </div>`;
    }
    if (report.content_url) {
        contentPreview += `<div style="background:#f8fafc;padding:8px;border-radius:8px;margin-bottom:6px;">
            <a href="${report.content_url}" target="_blank">🔗 ${report.content_url}</a>
        </div>`;
    }
    if (!contentPreview) {
        contentPreview = '<div style="color:#94a3b8;">内容已删除或无详细信息</div>';
    }

    const detailHtml = `
        <div style="margin-bottom:16px;">
            <div style="font-size:13px;color:#94a3b8;">举报人：<strong>${report.reporter_email || '未知'}</strong></div>
            <div style="font-size:13px;color:#94a3b8;margin-top:4px;">被举报内容分类：<strong>${report.content_category || '未知'}</strong></div>
            <div style="margin-top:8px;padding:12px;background:#f8fafc;border-radius:10px;border:1px solid #eef2f6;">
                <div style="font-weight:600;font-size:14px;color:#0f172a;">📖 内容预览</div>
                ${contentPreview}
            </div>
            <div style="font-size:13px;color:#94a3b8;margin-top:8px;">举报理由：</div>
            <div style="background:#fef3c7;padding:12px;border-radius:10px;margin-top:4px;font-size:14px;color:#92400e;">${report.reason}</div>
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap;">
            <button class="btn-sm approve" onclick="resolveReport(${report.id}, 'resolved', 'content_deleted')" style="padding:8px 16px;border-radius:10px;background:#dcfce7;color:#166534;border:1px solid #86efac;font-weight:600;">
                <svg viewBox="0 0 24 24" width="14" height="14" stroke="currentColor" fill="none" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 01-2 2H7a2 2 0 01-2-2V6m3 0V4a2 2 0 012-2h4a2 2 0 012 2v2"/></svg>
                删除内容
            </button>
            <button class="btn-sm" onclick="resolveReport(${report.id}, 'resolved', 'user_banned')" style="padding:8px 16px;border-radius:10px;background:#fee2e2;color:#991b1b;border:1px solid #fca5a5;font-weight:600;">
                <svg viewBox="0 0 24 24" width="14" height="14" stroke="currentColor" fill="none" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="4.93" y1="4.93" x2="19.07" y2="19.07"/></svg>
                封禁用户
            </button>
            ...
            <button class="btn-sm" onclick="resolveReport(${report.id}, 'dismissed', null)" style="padding:8px 16px;border-radius:10px;background:#f1f5f9;color:#475569;border:1px solid #e2e8f0;">
                <svg viewBox="0 0 24 24" width="14" height="14" stroke="currentColor" fill="none" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3"/></svg>
                驳回
            </button>
        </div>
    `;
...
async function banUser(userId, reason, banType, bannedBy) {
    ...
    // 强制踢出登录（通过更新profiles触发触发器）
    // 实际上触发器已经更新了profiles.is_banned
...
function renderAdminPage() {
    const html = `
        <div style="max-width:900px;margin:0 auto;">
            <h2 style="font-size:24px;color:#0f172a;margin-bottom:20px;">👑 管理中心</h2>
            ...
            <div id="adminTabContent">
                ...
                <div>
                    ...
                    <button onclick="submitAdminAction()" class="btn-submit" style="max-width:200px;">
...
function switchAdminTab(tab) {
    ...
    if (tab === 'announce') {
        let html = `
            ...
            <button onclick="publishGlobalAnnouncement()" class="btn-submit" style="max-width:200px;">
    ...
    content.innerHTML = `
        ...
        <div class="form-group">
            <label>目标用户ID</label>
            <input type="text" id="adminTargetId" placeholder="输入用户ID (UUID格式)" value="">
        </div>
        <div class="form-group" id="adminExtraField">
            <label>封禁时间（可选，不填为永久）</label>
            <input type="datetime-local" id="adminBanExpires">
...
async function submitAdminAction() {
    ...
    let targetId = document.getElementById('adminTargetId').value.trim();
    ...
    supabaseClient.from('profiles')
        ...
        .then(({ data }) => {
            ...
            const avatarHtml = data.avatar_url ?
                ...
                (data.nickname?.charAt(0) || 'U');

            // 检查修改限制
            let restrictionMsg = '';
            let canEdit = true;
            if (data.last_updated_at) {
                const last = new Date(data.last_updated_at);
                const now = new Date();
                const diff = Math.floor((now - last) / (1000 * 60 * 60 * 24));
                if (diff < 30) {
                    canEdit = false;
                    restrictionMsg = `<div class="restriction-msg">⏳ 距离下次修改还有 ${30 - diff} 天</div>`;
                }
            }

            // 统计信息
            const totalContents = allContents.filter(c => c.user_id === currentUser.id).length;
            const totalLikes = allContents.filter(c => c.user_id === currentUser.id).reduce((sum, c) => sum + (c.likes_count || 0), 0);
            const totalComments = allContents.filter(c => c.user_id === currentUser.id).reduce((sum, c) => sum + (c.comments_count || 0), 0);
                ...
                <div class="detail-container" style="max-width:700px;">
                    ...
                    <div class="detail-card">
                        <div class="profile-header">
                            <div class="big-avatar">${avatarHtml}</div>
                            <div class="profile-info">
                                <div class="name">${data.nickname || '用户'}</div>
                                <div class="id-text">🆔 ${data.id}</div>
                                ${data.bio ? `<div class="bio-text">${data.bio}</div>` : ''}
                                <div class="join-text">👤 加入于 ${timeAgo(data.created_at)}</div>
                                <div style="margin-top:8px;display:flex;gap:8px;flex-wrap:wrap;">
                                    <button class="btn-sm" onclick="openProfileEdit()" style="background:#6366f1;color:#fff;border:none;padding:4px 16px;border-radius:8px;">✏️ 编辑资料</button>
                                </div>
                                ${restrictionMsg}
                            </div>
                        </div>
                        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin:16px 0;padding:12px;background:#f8fafc;border-radius:12px;text-align:center;">
                            <div><div style="font-size:20px;font-weight:700;">${totalContents}</div><div style="font-size:12px;color:#94a3b8;">发布内容</div></div>
                            <div><div style="font-size:20px;font-weight:700;">${totalLikes}</div><div style="font-size:12px;color:#94a3b8;">获赞总数</div></div>
                            <div><div style="font-size:20px;font-weight:700;">${totalComments}</div><div style="font-size:12px;color:#94a3b8;">评论总数</div></div>
                        </div>
                        <div style="padding-top:16px;border-top:1px solid #f1f5f9;">
                            <h3 style="font-size:16px;color:#0f172a;margin-bottom:12px;">❤️ 我的收藏</h3>
                            ${collections.length === 0 ? '<p style="color:#94a3b8;font-size:14px;">暂无收藏</p>' : ''}
                            <div class="content-grid">
                                ${collections.map(c => {
                                    const item = c.contents;
                                    if (!item) return '';
                                    return `
                                        <div class="content-card" onclick="navigateTo('detail', ${item.id})" style="cursor:pointer;">
                                            <div class="title" style="font-size:15px;">${item.title}</div>
                                            <div class="meta" style="margin-top:6px;">
                                                <span class="cat ${getCategoryClass(item.category)}">${getCategoryIcon(item.category)} ${item.category}</span>
                                                <span>${timeAgo(item.created_at)}</span>
                        ...
                        <div style="padding-top:16px;border-top:1px solid #f1f5f9;margin-top:16px;">
                            <h3 style="font-size:16px;color:#0f172a;margin-bottom:12px;">👁️ 观看历史</h3>
                            ${watchHistory.length === 0 ? '<p style="color:#94a3b8;font-size:14px;">暂无观看历史</p>' : ''}
                            <div class="content-grid">
                                ${watchHistory.map(h => {
                                    const item = h.contents;
                                    if (!item) return '';
                                    return `
                                        <div class="content-card" onclick="navigateTo('detail', ${item.id})" style="cursor:pointer;">
                                            <div class="title" style="font-size:15px;">${item.title}</div>
...
//  初 始 化
// ============================================================
console.log('👁️ 必看网 v15.0.0 (全功能版)');
